...
const AlertComponent = () => {
    ...
    useEffect(() => {
        if (show) {
            setTimeout(() => {
                dispatch(setAlertBox({ title: "xxxxxx", message: "xxxxxx", color: "teal", show: false }))
            }, 2000)
        }
    })
    ...
    const cl = `bg-${color}-100 border-l-4 border-${color}-500 text-${color}-700 p-4 w-full h-15 text-${color}-700 px-4 py-6 rounded fixed`

    return (
        <>
            {
                show && <div className={cl} role="alert"
                style={{
                    background: 'red',
                    marginTop: 80,
                }}
